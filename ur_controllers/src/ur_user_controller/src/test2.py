...
class MoveGroupPythonIntefaceTutorial(object):
  ...
  def __init__(self):
    ...
    # 创建MoveGroupCommander的对象。MoveGroupCommander是针对一个规划组进行控制。
    # 这里通过设置group_name = panda_arm控制Panda机器人的手臂。
...
# 规划到一个姿态目标
  def go_to_pose_goal(self):
    ...
    rot_y = 0.2
    rot_z = 0.2
    pose_goal = [pos_x, pos_y, pos_z, rot_x, rot_y, rot_z]
...
# 在rviz中显示路径。调用group.plan()规划路径的时候会自动在rviz中显示
...
# 绑定物体到机械臂。
# 在用机械臂操纵物体的时候，为了防止MoveIt把某些link和物体的正常接触当做是碰撞而报，
# 可以把这些link加入到touch_links，这样就会忽略这些link和物体的碰撞。
